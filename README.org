...
- 29 :: Works, except it does not load the builtin dump. This means it's like running temacs directly, so you'll see a bunch of loading for builtin packages. Also no native compilation.
- 28 :: GUI crashes and does not open (same as 25). Does not load the builtin dump; no native compilation.
- 27 :: GUI crashes and does not open (same as 25). Does not load the builtin dump.
